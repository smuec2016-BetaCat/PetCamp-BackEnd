...
from models.adoptionOrder import AdoptionOrder, db
...
class AdoptionOrderAPI(Resource):
    """
    Adoption order API
    """
    @staticmethod
    ...
    def put():
        ...
        """
        json = request.get_json()
        order = AdoptionOrder.query.filter_by(ord_num=json["ord_num"]).first()
        if order is None:
    ...
    def get():
        ...
        """
        args = request.args
        if "owner_id" not in args and "ord_num" not in args:
            return {"error": "Either owner ID or order number must be provided"}, 406, acao
        if "ord_num" not in args:
            orders = AdoptionOrder.query.all()
            for i in range(len(orders)):
                orders[i] = to_dict(orders[i])
            return {"orders": orders}, 200, acao
        order = AdoptionOrder.query.filter_by(ord_num=args["ord_num"]).first()
        if order is None:
